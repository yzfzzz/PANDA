...
class Panda_tool:
    ...
    CATEGORIES = [{'id': 1, 'name': 'visible car'},  # 车体可见部分
                  {'id': 2, 'name': 'person'},  # 全人体
                  ]
    ...
    def split_train_val(json_path: str, pic_path: str, ratio: float = 0.2):
        ...
        for image_name in tqdm(img_instance.keys()):
            ...
            image_anno['file_name'] = image_name.split('/',2)[-1]
    ...
    @staticmethod
    def bbox_json2commited_4divided(coco_test_path: str, bbox_json_path: str,
        ...
        with open('panda_round1_test_A_annos_202104/person_bbox_test.json','r') as fr:
            img2wh_id = json.load(fr)
    ...
    def run():
        ...
        #从切分图还原到原图
        # Panda_tool.bbox_json2commited_4divided(coco_test_path='./panda_round1_coco_full_patches_wh_3000_3000_testA.json',
        #                                                  bbox_json_path='./panda_patches_results.bbox.json',
        #                                                  save_json_path='./commited_result.json',overlap_factor=[1500,1500],
        #                                                   nms_thresh=0.5,use_box_voting=True,
        #                                                  show_flag=False)

        # 对结果进行可视化操作
        Panda_tool.vis_gt_offical(json_path='./results/AP0.47_AR0.57_score0.514_1030/results/det_results.json',img_root_path='panda_round1_test_202104_A')

        # 划分训练集、验证集
        Panda_tool.split_train_val(json_path='./panda_round1_coco_full.json',
                                   pic_path='./panda_round1_train_202104',ratio=0.2)
